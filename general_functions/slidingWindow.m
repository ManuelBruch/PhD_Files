...
% 
% rates.av                =   zeros(steps_default,size(y,2));
% rates.stdev             =   rates.av;
% for i = 1:length(data.conditions_single)
%     rates.av(:,i)           =   mean(rates.coefficients.m(:,data.C{1,i}),2);
%     rates.stddev(:,i)       =   std(rates.coefficients.m(:,data.C{1,i}),0,2);
% end
% 
% rates.coefficients.m    =   rates.av;
% rates.standardError     =   rates.stddev;
% rates.average_timepoint =   data.time.hours(ceil(width/2):end-floor(width/2));
